...
import {Vulcan, VulcanInterface, VulcanMessage, String, requiredParamsValid} from "vulcan-ts";
...
export class AgoraLib {

    //--------------------------------------- AgoraLib Consts ------------------------------ //
    static readonly IS_ENTITLED = "IsEntitled";
    static readonly GET_PURCHASE_URL = "GetPurchaseUrl";
    static readonly CREATE_ENTITLEMENT = "CreateEntitlement";
    static readonly GET_VERSION = "GetVersion";
    static readonly STATUS = "Status";
    static readonly STATUSCODE = "StatusCode";
    static readonly MESSAGE_TYPE = "vulcan.SuiteMessage.cosy.exchangeplugin.ApiRequest";
    static readonly RESPONSE_TYPE = "vulcan.SuiteMessage.cosy.exchangeplugin.ApiResponse";
    static readonly ACCC_STARTED_UP_MESSAGE = "vulcan.SuiteMessage.any.cosy.app.StartedUp";
    static readonly MESSAGE_REQUEST_TEMPLATE = "<Message><Requests>{0}</Requests></Message>";
    static readonly MESSAGE_RESPONSE_TEMPLATE = "<Message><Responses>{0}</Responses></Message>";
    ...
    extensionID = (<any>window).__adobe_cep__.getExtensionId();
    ...
    constructor() {
        ...
        this.bundleID = (<any>window).__adobe_cep__.invokeSync("getBundleId", "");
    }

//--------------------------------------- Private functions ------------------------------    
    private responseCallback(message: any): void {
        var that = this;
    ...
    private checkConnection(callback: (executionResult: any) => void, ignoreACCC: boolean): void {
        var that = this;
    ...
    private compareVersion(a: any, b: any): number {
        var i, cmp, len, re = /(\.0)+[^\.]*$/;
...
//--------------------------------------- Public API ------------------------------
    ...
    isEntitled(callback, ignoreACCC) {
    ...
    getPurchaseUrl(callback, straightToCheckout, ignoreACCC) {
        if (typeof(Vulcan) === 'undefined') {
            throw 'Vulcan.js is required.';
        }
    ...
    createEntitlement(callback: (executionResult: any) => void, ignoreACCC: boolean): void {
        ...
        this.checkConnection(function (responseObj) {
            ...
            if (responseObj.statusCode === "0" && that.compareVersion(responseObj.response, "1.0.0") >= 0) {
                ...
                var csLibrary:any = new CSInterface();
                ...
                var fileContents = (<any>window).cep.fs.readFile(manifestPath);
                var error = true;
                if ((<any>window).DOMParser) {
                    var parser = new (<any>window).DOMParser();
                    try {
