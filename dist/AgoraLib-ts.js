...
/**************************************************************************************************
 /**
 *  Typescript implementation of AgoraLib - v6.1.0
 *
 *  Custom AgoraLib.js implementation in Typescript.
 *  The implementation covers version 6.1.x from original repository, with all functionalities.
...
var vulcan_ts_1 = require("vulcan-ts");
/** AgoraLib - v6.1.0 */
/**
 * @class AgoraLib
 *
...
var AgoraLib = /** @class */ (function () {
    function AgoraLib() {
        ...
        this.callerID = this.guid + '_' + this.extensionID;
        this.bundleID = window.__adobe_cep__.invokeSync("getBundleId", "");
    }
    //--------------------------------------- Private functions ------------------------------    
    AgoraLib.prototype.responseCallback = function (message) {
        var that = this;
        if (window.DOMParser) {
                ...
                for (var i = 0; i < responses.length; i++) {
                    ...
                    if (respElem) {
                        ...
                        if (callerID === that.callerID) {
                            ...
                            vulcan_ts_1.VulcanInterface.removeMessageListener(AgoraLib.RESPONSE_TYPE, that.responseCallback);
                            // extract parameters
    ...
    ;
    AgoraLib.prototype.checkVersion = function () {
        var that = this;
        console.log("Checking version...");
        var request = "<Request name=\"" + AgoraLib.GET_VERSION + "\" callerID=\"" + that.callerID + "\"><Parameters>";
        request += "</Parameters></Request>";
        var sub_payload = vulcan_ts_1.String.format(AgoraLib.MESSAGE_REQUEST_TEMPLATE, request);
        var GetVersionVulcanMessage = new vulcan_ts_1.VulcanMessage(AgoraLib.MESSAGE_TYPE);
        GetVersionVulcanMessage.setPayload(sub_payload);
        vulcan_ts_1.VulcanInterface.addMessageListener(AgoraLib.RESPONSE_TYPE, that.responseCallback);
        vulcan_ts_1.VulcanInterface.dispatchMessage(GetVersionVulcanMessage);
        return;
    };
    ;
    AgoraLib.prototype.continueWithCheckVersion = function () {
        var that = this;
    ...
    ;
    AgoraLib.prototype.checkConnection = function (callback, ignoreACCC) {
        var that = this;
        ...
        var isInstalled = vulcan_ts_1.VulcanInterface.isAppInstalled("coresync");
        if (!isInstalled) {
            ...
            var isRunning = vulcan_ts_1.VulcanInterface.isAppRunning("coresync");
            ...
            if (!isRunning && !ignoreACCC) {
                ...
                isRunning = vulcan_ts_1.VulcanInterface.launchApp("creativecloud", false, "");
                if (isRunning) {
                    console.log("ACCC is now being launched");
                    var startedUpCallback = function (message) {
                        console.log("ACCC started up");
                        vulcan_ts_1.VulcanInterface.removeMessageListener(AgoraLib.ACCC_STARTED_UP_MESSAGE, this);
                    ...
                    vulcan_ts_1.VulcanInterface.addMessageListener(AgoraLib.ACCC_STARTED_UP_MESSAGE, startedUpCallback);
                    return;
                }
                else {
    ...
    AgoraLib.prototype.compareVersion = function (a, b) {
        var i, cmp, len, re = /(\.0)+[^\.]*$/;
     ...
     * @since 5.2.0
     */
    AgoraLib.prototype.isEntitled = function (callback, ignoreACCC) {
        if (typeof (vulcan_ts_1.Vulcan) === 'undefined') {
        ...
        this.checkConnection(function (responseObj) {
            ...
            if (responseObj.statusCode === "0" && that.compareVersion(responseObj.response, "1.0.0") >= 0) {
                ...
                request += "</Parameters></Request>";
                var sub_payload = vulcan_ts_1.String.format(AgoraLib.MESSAGE_REQUEST_TEMPLATE, request);
                var isEntitledVulcanMessage = new vulcan_ts_1.VulcanMessage(AgoraLib.MESSAGE_TYPE);
                isEntitledVulcanMessage.setPayload(sub_payload);
                vulcan_ts_1.VulcanInterface.addMessageListener(AgoraLib.RESPONSE_TYPE, that.responseCallback);
                vulcan_ts_1.VulcanInterface.dispatchMessage(isEntitledVulcanMessage);
            }
            else {
     ...
     * @since 5.2.0
     */
    AgoraLib.prototype.getPurchaseUrl = function (callback, straightToCheckout, ignoreACCC) {
        if (typeof (vulcan_ts_1.Vulcan) === 'undefined') {
            throw 'Vulcan.js is required.';
        }
        if (callback === null || callback === undefined) {
        ...
        straightToCheckout = (!vulcan_ts_1.requiredParamsValid(straightToCheckout) || straightToCheckout === '') ? false : straightToCheckout;
        var that = this;
        var success = false;
        // check connection
        this.checkConnection(function (responseObj) {
            ...
            if (responseObj.statusCode === "0" && that.compareVersion(responseObj.response, "1.0.0") >= 0) {
                ...
                request += "</Parameters></Request>";
                var sub_payload = vulcan_ts_1.String.format(AgoraLib.MESSAGE_REQUEST_TEMPLATE, request);
                var getPurchaseUrlVulcanMessage = new vulcan_ts_1.VulcanMessage(AgoraLib.MESSAGE_TYPE);
                getPurchaseUrlVulcanMessage.setPayload(sub_payload);
                vulcan_ts_1.VulcanInterface.addMessageListener(AgoraLib.RESPONSE_TYPE, that.responseCallback);
                vulcan_ts_1.VulcanInterface.dispatchMessage(getPurchaseUrlVulcanMessage);
            }
            else {
     ...
     * @since 5.2.0
     */
    AgoraLib.prototype.createEntitlement = function (callback, ignoreACCC) {
        if (typeof (vulcan_ts_1.Vulcan) === 'undefined') {
            throw 'Vulcan.js is required.';
        }
        if (typeof (csinterface_ts_1.CSInterface) === 'undefined') {
        ...
        this.checkConnection(function (responseObj) {
            ...
            if (responseObj.statusCode === "0" && that.compareVersion(responseObj.response, "1.0.0") >= 0) {
                ...
                var csLibrary = new csinterface_ts_1.CSInterface();
                var extensionDir = csLibrary.getSystemPath(csinterface_ts_1.SystemPath.EXTENSION);
                ...
                if (window.DOMParser) {
                    ...
                    try {
                        ...
                        if (extManifest) {
                            ...
                            request += "</Parameters></Request>";
                            var sub_payload = vulcan_ts_1.String.format(AgoraLib.MESSAGE_REQUEST_TEMPLATE, request);
                            var createEntitlementVulcanMessage = new vulcan_ts_1.VulcanMessage(AgoraLib.MESSAGE_TYPE);
                            createEntitlementVulcanMessage.setPayload(sub_payload);
                            vulcan_ts_1.VulcanInterface.addMessageListener(AgoraLib.RESPONSE_TYPE, that.responseCallback);
                            vulcan_ts_1.VulcanInterface.dispatchMessage(createEntitlementVulcanMessage);
                            error = false;
                        }
                    }
                    catch (e) {
 ...
 */
var AgoraLibResponse = /** @class */ (function () {
    function AgoraLibResponse(response, status, statusCode) {
        this.response = response;
        this.status = status;
        this.statusCode = statusCode;
    }
    return AgoraLibResponse;
}());
exports.AgoraLibResponse = AgoraLibResponse;
